...
function doPost(e) {
  try {
    ...
    if (data.action === "deleteLodging") {
      deleteLodging(ss, data);
    }

    if (data.action === "deleteScheduleEvent") {
      deleteScheduleEvent(ss, data);
      return ContentService
        .createTextOutput(JSON.stringify({ success: true }))
        .setMimeType(ContentService.MimeType.JSON);
    }
...
function deleteScheduleEvent(ss, d) {
  ...
  var headers = data[0];
  var tourGroupCol = headers.indexOf("Tour Group");
  var dateCol = headers.indexOf("Date");
  var eventCol = headers.indexOf("Event");

  var incomingDate = normalizeDate(d.date);
  for (var i = data.length - 1; i >= 1; i--) {
    var sheetDate = normalizeDate(String(data[i][dateCol]));
    var rowEvent = String(data[i][eventCol]);
    var rowGroup = String(data[i][tourGroupCol]);
    if (rowGroup === d.tourGroup && sheetDate === incomingDate && rowEvent === d.originalEventType) {
      sheet.deleteRow(i + 1);
      break;
    }
  }
...
function updateScheduleRow(ss, d) {
  ...
  var tourGroupCol = headers.indexOf("Tour Group");
  var dateCol = headers.indexOf("Date");
  var eventTypeCol = headers.indexOf("Event");

  var incomingDate = normalizeDate(d.date);
  
  for (var i = 1; i < data.length; i++) {
    var sheetDate = normalizeDate(String(data[i][dateCol]));
    if (String(data[i][tourGroupCol]) === d.tourGroup &&
        sheetDate === incomingDate &&
        String(data[i][eventTypeCol]) === d.originalEventType) {
      sheet.getRange(i + 1, eventTypeCol + 1).setValue(d.eventType || "");
      sheet.getRange(i + 1, headers.indexOf("Destination") + 1).setValue(d.churchName || "");
      sheet.getRange(i + 1, headers.indexOf("Destination City, State") + 1).setValue(d.cityState || "");
      sheet.getRange(i + 1, headers.indexOf("Pastor") + 1).setValue(d.pastor || "");
      sheet.getRange(i + 1, headers.indexOf("Event Time") + 1).setValue(d.serviceTimes || "");
      sheet.getRange(i + 1, headers.indexOf("Leader Notes") + 1).setValue(d.notes || "");
      break;
    }
  }
}
